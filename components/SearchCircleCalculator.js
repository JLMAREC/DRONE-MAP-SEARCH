...
export default function SearchCircleCalculator({ onRadiusCalculated, victimLocation }) {
  ...
  const calculateAdjustedSpeed = () => {
    ...
    const terrainFactors = {
      normal: 1,      // Normal (plat)
      denivele: 0.75, // Dénivelé : -25%
      foret: 0.65     // Forêt : -35%
    };
    ...
    const victimFactors = {
      normal: 1,     // État normal
      blesse: 0.7,   // Blessé : -30%
      panique: 0.85  // Panique : -15%
    };
    ...
    const radius = speed * hoursElapsed * 1000; // Conversion en mètres
    return { speed, radius };
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <button
        type="submit"
        className={`w-full mt-4 text-white py-2 px-4 rounded-md transition-colors ${
          victimLocation 
            ? 'bg-[#1a2742] hover:bg-[#243150] focus:ring-2 focus:ring-[#1a2742] focus:ring-offset-2'
